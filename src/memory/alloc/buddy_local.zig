...
const get_buddy = @import("utils.zig").get_buddy;
...
pub const CpuLocalBuddyAllocator = struct {
    ...
    fn collect_from_remote_list(self: *Self) void {
        const rfl: *?*Page = &self.remote_free_list;
        var head = @atomicRmw(?*Page, rfl, .Xchg, null, .acquire);
        while (head) |page| {
    ...
    fn internal_free(self: *Self, page: *Page) void {
        ...
        var current_pfn: u64 = @intFromPtr(get_addr_from_page(page, self.metadata_chunks)) / 4096;
        for (page.order..AllocationSize._4MB.to_buddy_size()) |current_order| {
            const buddy_pfn = current_pfn ^ (@as(u64, 1) << @intCast(current_order));
            const buddy_ptr: *Page = get_page(self.metadata_chunks, @ptrFromInt(buddy_pfn * 4096)) orelse break;
            ...
            if (buddy_ptr.flags.is_free and buddy_ptr.owner_id == self.cpu_id and buddy_ptr.order == current_order) {
                ...
                const current_order_u8: u8 = @intCast(current_order);
                final_order = current_order_u8 + 1;
            } else {
                break;
            }
        }
        const final_page_ptr = get_page(self.metadata_chunks,@ptrFromInt(current_pfn * 4096));
    ...
    fn refill(self: *Self, target: AllocationSize) void {
        ...
        // slow way
        {
            var addrs = [4]?*anyopaque{null, null, null, null};
            ...
            if (is_success) {
                for (addrs) |maybe_addr| {
                    if (maybe_addr) |addr| {
                        const page: *Page = get_page(self.metadata_chunks, addr) orelse @panic("hell 100");
                        const current_order_u6: u6 = @intCast(current_order);
                        const pages_in_block = @as(usize, 1) << current_order_u6 - (@intFromEnum(AllocationSize._4KB));
    ...
    fn send_remote(_: *Self, page: *Page) void {
        const owner_id = page.owner_id;
        const target_data = CPU_DATA[@intCast(owner_id)] orelse @panic("hell 140");
    ...
    pub fn alloc(self: *Self, order: AllocationSize) ?*anyopaque {
        ...
        for (order.to_buddy_size()..AllocationSize._4MB.to_buddy_size() + 1) |current_order| {
            if (!self.free_lists[current_order].isEmpty()) {
                ...
                const page: *Page = self.free_lists[current_order].pop_front() orelse @panic("hell 164");
                assert(page.check_magic());
                assert(page.owner_id == self.cpu_id);
                if (!page.flags.no_owner) {
                    const pages: [*]Page = @ptrCast(page);
                    var current = current_order - @as(usize, 1);
                    const end = order.to_buddy_size();
                    while (true) : (current -= 1) {
                        const current_u6: u6 = @intCast(current);
                        const sec_page = &pages[@as(usize, 1) << current_u6];
                        sec_page.order = @intCast(current);
                    ...
                    const no_owner = page.flags.no_owner;
                    page.flags = .{.no_owner = no_owner, .is_free = false};
                    page.order = order.to_buddy_size();
                    return get_addr_from_page(page, self.metadata_chunks);
                } else {
                    self.free_lists[current_order].push_front(page);
                }
            }
        }
        return null;
