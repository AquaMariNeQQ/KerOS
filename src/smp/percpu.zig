const InterruptController = @import("../platform_info.zig").InterruptController;
...
pub const PerCpuData = struct {
    ...
    local_ic: InterruptController,

    // -----------------------------

    fn new(cpu_id: u8) *PerCpuData {
        const phys: *anyopaque = make: {
            const alloc = ALLOCATOR.lock();
            defer ALLOCATOR.unlock();
            break :make alloc.alloc(._4KB) orelse @panic("OOM");
        };
